...
then
    ...
    # Setup Keycloak Social App if OIDC is enabled (sempre eseguito)
    if [ "${SOCIALACCOUNT_OIDC_PROVIDER_ENABLED}" = "true" ] || [ "${SOCIALACCOUNT_OIDC_PROVIDER_ENABLED}" = "True" ]; then
        echo "🔐 Setting up Keycloak Social App..."
        python manage.py setup_keycloak --no-input || echo "⚠️  Keycloak setup failed or already configured"
    fi
