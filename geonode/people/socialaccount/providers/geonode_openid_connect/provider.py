...
# Compatibility fix for different versions of django-allauth
try:
    from allauth.socialaccount.providers.openid_connect.provider import OpenIDConnectAccount
except ImportError:
    # Fallback for newer versions of django-allauth
    try:
        from allauth.socialaccount.providers.openid_connect.provider import OpenIDConnectProviderAccount as OpenIDConnectAccount
    except ImportError:
        # Final fallback if neither exists
        OpenIDConnectAccount = ProviderAccount
...
class GenericOpenIDConnectProvider(OAuth2Provider):
    ...
    # Safe loading of account class with fallback
    @property
    def account_class(self):
        ...
        try:
            return import_string(account_class_path)
        except ImportError:
            # Fallback to the local class if import fails
            return GenericOpenIDConnectProviderAccount
